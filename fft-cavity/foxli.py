...
# def propagate(field, dist, lda, Freqs=None, dx=None):
#     if Freqs is None:
#         N = field.shape[0]
#         freqx = nf.fftfreq(N, dx)
#         Freqs = np.meshgrid(freqx, freqx)
#
#     phase_shift = 2*np.pi/lda * dist - np.pi * lda * dist * (Freqs[0]**2 + Freqs[1]**2)
#     return nf.ifft2(nf.fft2(field) * np.exp(-1j*phase_shift))
...
for rt_num in range(10):
    in_field = cavity_fun(in_field)
...
fig, ax = plt.subplots()
ax.imshow(abs(in_field)**2)

fig, (axI, axP) = plt.subplots(2, 1)
plot_field(in_field, axI=axI, axP=axP)
