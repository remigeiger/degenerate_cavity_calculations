...
fig, ax = plt.subplots()
for field1, field2 in zip(handler.fields[:5,:], handler.fields[1:6, :]):
    phase1 = np.unwrap(np.angle(field1))
    phase2 = np.unwrap(np.angle(field2))

    phase1 -= phase1[nx//2]
    phase2 -= phase2[nx//2]

    ax.plot(phase2 - phase1)
