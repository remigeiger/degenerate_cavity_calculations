...
# config = dict(waist=5.0e-3, orders=[(4, 0)], coeffs=[1.3*2*np.pi], delta=-1850e-6)
config = dict(waist=3.6e-3, orders=[(4, 0)], coeffs=[1.3*2*np.pi], delta=-1850e-6)
config = dict(waist=3.6e-3, orders=[(4, 0)], coeffs=[0.5*1.3*2*np.pi], delta=-925e-6)
config = dict(waist=3e-3, orders=[], coeffs=[], delta=0)
...
folder_name = '{:%Y_%m_%d_%H_%M_%S}/'.format(datetime.datetime.today())
...
def fermi_dirac(r, R, beta, amplitude):
    E = np.exp(-beta*(r/R - 1))
...
E = fermi_dirac(rs, w, 14, 1.0).astype(np.complex128)
E /= np.sqrt(np.trapz(x=rs, y=rs * abs(E) ** 2))
...
#        E_p *= apo
    ...
    ax_spectrum.plot((phases)/(2*np.pi), spectrum, label=r'{:.1f} $\mu$m'.format((d2 - delta_quadratic_compensation)*1e6))
